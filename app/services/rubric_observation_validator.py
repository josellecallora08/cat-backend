...
def _schema_errors(exc: ValidationError) -> list[ValidationIssue]:
    ...
    for error in exc.errors():
        ...
        code = "required" if error_type == "missing" else "invalid_evidence" if "evidence" in location else "invalid"
...
def _transcript_index(transcript: list[dict[str, Any]]) -> dict[int, dict[str, Any]]:
    ...
    for index, entry in enumerate(transcript):
        ...
        if sequence in indexed:
            errors.append(_error("duplicate", f"transcript[{index}].sequence_number", "Sequence number is duplicated."))
...
def validate_observation(
    ...
    try:
        parsed = observation if isinstance(observation, RubricAIObservation) else RubricAIObservation.model_validate(observation)
        rubric = snapshot if isinstance(snapshot, NegotiationStandardContent) else NegotiationStandardContent.model_validate(snapshot)
    ...
    if len(category_ids) != len(set(category_ids)):
        errors.append(_error("duplicate", "categories", "Each rubric block must appear exactly once."))
    expected = set(blocks)
    ...
    for unknown in sorted(actual - expected):
        errors.append(_error("unknown_reference", "categories", f"Unknown rubric block '{unknown}'."))
    ...
    for technique_index, technique in enumerate(parsed.applied_techniques.techniques_used):
        ...
        if technique.technique_name.casefold() not in valid_technique_names:
            errors.append(_error("unknown_reference", f"{path}.technique_name", "Technique is not declared in the published rubric."))
        for sequence_index, sequence in enumerate(technique.evidence_sequence_numbers):
            if sequence not in transcript_by_sequence:
                errors.append(_error("unknown_reference", f"{path}.evidence_sequence_numbers[{sequence_index}]", "Transcript sequence does not exist."))
    ...
    if errors:
        raise ObservationValidationError(sorted(errors, key=lambda item: (item.path, item.code, item.message)))
...
def _validate_sequences(
    ...
    for index, sequence in enumerate(sequences):
        if sequence not in evidence_sequences:
            errors.append(_error("required", f"{path}[{index}]", "Finding requires matching category evidence."))
...
def _validate_evidence(
    ...
    if entry is None:
        errors.append(_error("unknown_reference", f"{path}.sequence_number", "Transcript sequence does not exist."))
        return
    if entry.get("speaker") != evidence.speaker:
        errors.append(_error("invalid_evidence", f"{path}.speaker", "Evidence speaker does not match the persisted transcript."))
    if _normalize(evidence.excerpt) not in _normalize(str(entry.get("text", entry.get("utterance_text", "")))):
        errors.append(_error("invalid_evidence", f"{path}.excerpt", "Evidence excerpt is not contained in the persisted utterance."))
