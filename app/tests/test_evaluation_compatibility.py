...
SNAPSHOT = {
    ...
    "blocks": [{
        ...
        "positive_behaviors": [{"id": "move-forward", "name": "Move Forward", "description": "Redirects.", "evidence_instructions": "Quote."}],
        "violations": [{"id": "legal-threat", "name": "Legal Threat", "description": "Threatens.", "evidence_instructions": "Quote."}],
...
def _canonical():
    observation = {
        ...
        "categories": [{
            ...
            "raw_score": 60,
            "evidence": [{"sequence_number": 2, "speaker": "agent", "excerpt": "We can discuss options", "explanation": "The agent discussed options."}],
            "strengths": [{"criterion_id": "move-forward", "explanation": "The agent redirected the discussion.", "evidence_sequence_numbers": [2]}],
            "violations": [{"violation_id": "legal-threat", "explanation": "The agent made a legal threat.", "evidence_sequence_numbers": [2]}],
        ...
        "applied_techniques": {"techniques_used": [{"technique_name": "Move Forward", "execution_type": "Executed Properly", "execution_description": "The agent redirected the discussion.", "evidence_sequence_numbers": [2]}], "reason_if_empty": "None."},
        "missed_opportunities": {"missed_techniques": [{"technique_name": "Legal Threat", "reason": "A compliant alternative was needed."}], "reason_if_empty": "None."},
    }
    validated = validate_observation(observation, SNAPSHOT, [{"sequence_number": 2, "speaker": "agent", "text": "We can discuss options"}])
...
def test_legacy_context_contains_only_derived_template_variables() -> None:
    ...
    assert context["missed_opportunities"]["missed_techniques"][0]["technique_name"] == "Legal Threat"
...
def test_recommendations_redact_names_organizations_amounts_emails_and_locations() -> None:
    ...
    canonical.categories[0].violations[0].explanation = (
        "John from Acme in Manila owes $5,000; contact john@example.com."
    )
