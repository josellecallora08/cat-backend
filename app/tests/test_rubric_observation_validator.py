...
def _assert_invalid(observation: dict) -> set[str]:
    with pytest.raises(ObservationValidationError) as caught:
        validate_observation(
            observation,
            SNAPSHOT,
            [{"sequence_number": 1, "speaker": "agent", "text": "Hello there"}],
        )
...
def test_rejects_missing_transcript_evidence_and_mismatched_speaker_or_excerpt() -> None:
    ...
    wrong_excerpt = _category(
        evidence=[
            {"sequence_number": 1, "speaker": "agent", "excerpt": "Not present", "explanation": "x"}
        ]
    )
...
def test_rejects_unknown_references_and_findings_without_category_evidence() -> None:
    category = _category(
        evidence=[],
        strengths=[
            {"criterion_id": "greeting", "explanation": "x", "evidence_sequence_numbers": [1]}
        ],
        violations=[
            {"violation_id": "unknown", "explanation": "x", "evidence_sequence_numbers": [1]}
        ],
...
def test_whitespace_normalization_is_not_fuzzy_matching() -> None:
    ...
    validate_observation(
        _observation(categories=[category]),
        SNAPSHOT,
        [{"sequence_number": 1, "speaker": "agent", "text": " Hello there "}],
    )
    assert "invalid_evidence" in _assert_invalid(
        _observation(
            categories=[
                _category(
                    evidence=[
