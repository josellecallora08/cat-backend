...
@pytest.mark.asyncio
async def test_complete_campaign_to_pinned_evaluation_and_results_api(
    ...
    await create_standard(
        db_session, campaign.id, admin.id, "Collection standard", None, _content()
    )
    ...
    assert (
        stored_coaching.mistakes_by_category["_rubric_coaching"]["blocks"][0]["recommendations"][0][
            "criterion_name"
        ]
        == "Rude tone"
    )
    ...
    try:
        ...
        assert (
            coaching_response.status_code == 200
            and coaching_response.json()["rubric_recommendations"]
        )
        ...
        assert (
            coaching_data["rubric_coaching"]["blocks"][0]["recommendations"][0]["criterion_name"]
            == "Rude tone"
        )
        assert plan_response.status_code == 200 and plan_response.json()[
            "standard_version_id"
        ] == str(version.id)
    finally:
