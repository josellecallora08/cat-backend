...
def _content(
    weight: int = 100, *, guidance: str = "Use respectful language."
...
@pytest.mark.asyncio
async def test_authorization_matrix_rejects_every_standard_operation_without_state_change(
    api_database,
) -> None:
    ...
    routes = [
        ...
        (
            ...
            {"name": "Other", "draft_content": _content().model_dump(mode="json")},
        ),
        (
            "put",
            f"/api/campaigns/{campaign.id}/negotiation-standard",
...
async def test_admin_can_complete_lifecycle_and_duplicate_publish_is_stable(api_database) -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        created = await client.post(
            base, json={"name": "Collection", "draft_content": _content().model_dump(mode="json")}
        )
        ...
        updated = await client.put(
            base, json={"expected_revision": revision, "name": "Collection v1"}
        )
...
async def test_publication_rejects_boundary_weights_that_do_not_total_one_hundred(
    db_session: AsyncSession, weight: int
) -> None:
    admin = User(
        id=uuid4(),
        email=f"{uuid4()}@example.test",
...
@pytest.mark.asyncio
async def test_invalid_weight_101_is_rejected_and_exact_100_is_publishable(
...
@pytest.mark.asyncio
async def test_published_snapshot_is_immutable_and_history_does_not_change(
    db_session: AsyncSession,
) -> None:
    admin = User(
        id=uuid4(),
    ...
    version = (
        await db_session.execute(
            select(NegotiationStandardVersion).where(NegotiationStandardVersion.id == version_id)
        )
    ).scalar_one()
...
async def test_v1_v2_sessions_keep_pinned_snapshots_after_archive(db_session: AsyncSession) -> None:
    ...
    await create_standard(
        ...
        "Collection",
        None,
        _content(guidance="Version one guidance."),
    )
    version_one = await publish_standard(db_session, campaign.id, admin.id)
    session_a = await create_session(
        db_session, scenario.id, agent.id, _FakeSimulator(), campaign.id
    )
    ...
    db_session.add_all(
        [
            Evaluation(
                ...
                strengths=[],
                weaknesses=[],
                negotiation_standard_version_id=version_one.id,
                standard_snapshot=version_one.snapshot,
                weighted_total=80,
                passing_score=70,
                passed=True,
            ),
            Evaluation(
                ...
                strengths=[],
                weaknesses=[],
                negotiation_standard_version_id=version_two.id,
                standard_snapshot=version_two.snapshot,
                weighted_total=90,
                passing_score=70,
                passed=True,
            ),
    ...
    assert (
        session_a.negotiation_standard_version.snapshot["blocks"][0]["recommendation_guidance"]
        == "Version one guidance."
    )
    assert (
        session_b.negotiation_standard_version.snapshot["blocks"][0]["recommendation_guidance"]
        == "Version two guidance."
    )
    results = (
        (await db_session.execute(select(Evaluation).order_by(Evaluation.overall_score)))
        .scalars()
        .all()
    )
    assert [
        (
            item.negotiation_standard_version_id,
            item.standard_snapshot["blocks"][0]["recommendation_guidance"],
        )
        for item in results
    ] == [
...
@pytest.mark.parametrize(
    ...
    [
        {
            "evidence": [
                {"sequence_number": 99, "speaker": "agent", "excerpt": "Hello", "explanation": "x"}
            ]
        },
        {
            "evidence": [
...
def test_evidence_integrity_rejects_missing_fabricated_and_wrong_speaker_evidence(
    change: dict,
) -> None:
...
def test_recommendation_integrity_and_redaction_are_enforced() -> None:
        ...
        categories=[
            {
                **_observation()["categories"][0],
                "evidence": [
                    {
                        ...
                        "speaker": "agent",
                        "excerpt": "Hello there",
                        "explanation": "John from Acme in Manila owes $5,000; john@example.com.",
                    }
                ],
                "violations": [
                    {
                ...
                "recommendation_inputs": [
                    {
                        "criterion_id": "rude-tone",
                        "transcript_sequence_number": 1,
                        "need": "Use a safer response.",
                    }
                ],
            }
        ]
    ...
    assert all(
        value not in text for value in ("John", "Acme", "Manila", "$5,000", "john@example.com")
    )
...
def test_applied_and_missed_techniques_are_disjoint() -> None:
    observation = _observation(
        ...
        missed_opportunities={
            "missed_techniques": [{"technique_name": "Greeting", "reason": "Not missed."}],
            "reason_if_empty": "None.",
        },
    )
    with pytest.raises(ObservationValidationError):
...
async def test_prompt_injection_output_is_rejected_with_no_protected_prompt_leak() -> None:
    ...
    transcript = [
        {"sequence_number": index, "speaker": "agent", "text": text}
...
@pytest.mark.asyncio
async def test_api_rejects_stale_and_published_mutations_without_history_changes(
    api_database,
) -> None:
    admin, campaign, factory = api_database
