            ...
            "blocks": [
                {
                    ...
                    "penalties": [
                        {"violation_id": "rushed-tone", "deduction": 10, "max_occurrences": 1}
                    ],
                ...
                },
                {
                    ...
                    "penalties": [
                        {"violation_id": "no-plan", "deduction": 15, "max_occurrences": 1}
                    ],
...
def _observation() -> dict:
    return {
        ...
        "categories": [
            {
                ...
                "evidence": [
                    {
                        "sequence_number": 0,
                        "speaker": "agent",
                        "excerpt": "Hello, this is Alex",
                        "explanation": "A clear greeting.",
                    },
                    {
                        "sequence_number": 2,
                        "speaker": "agent",
                        "excerpt": "We need payment today",
                        "explanation": "The wording was rushed.",
                    },
                ],
                "strengths": [
                    {
                        "criterion_id": "greeting",
                        "explanation": "The call began clearly.",
                        "evidence_sequence_numbers": [0],
                    }
                ],
                "violations": [
                    {
                        "violation_id": "rushed-tone",
                        "explanation": "The demand was rushed.",
                ...
                "recommendation_inputs": [
                    {
                        "criterion_id": "rushed-tone",
                        "transcript_sequence_number": 2,
                        "need": "Use a calmer opening.",
                    }
                ],
            },
            {
                ...
                "evidence": [
                    {
                        "sequence_number": 6,
                        "speaker": "agent",
                        "excerpt": "The full amount is due",
                        "explanation": "No flexible option was offered.",
                    }
                ],
                "strengths": [],
                "violations": [
                    {
                        "violation_id": "no-plan",
                        "explanation": "The agent did not offer a plan.",
                        "evidence_sequence_numbers": [6],
                    }
                ],
                "failed_criteria": ["no-plan"],
                "recommendation_inputs": [
                    {
                        "criterion_id": "no-plan",
                        "transcript_sequence_number": 6,
                        "need": "Offer a realistic plan.",
                    }
                ],
            },
        ],
        "applied_techniques": {
            "techniques_used": [
                {
                    "technique_name": "Greeting",
                    "execution_type": "Executed Properly",
                    "execution_description": "The greeting was clear.",
                    "evidence_sequence_numbers": [0],
                }
            ],
        ...
        "missed_opportunities": {
            "missed_techniques": [],
            "reason_if_empty": "No missed opportunities recorded.",
        },
    }
...
async def _seed_identity(
    db: AsyncSession, *, campaign: bool = True
) -> tuple[User, Scenario, Campaign | None, object | None]:
    agent = User(
        id=uuid.uuid4(),
        email=f"{uuid.uuid4()}@e2e.test",
        full_name="E2E Agent",
        role="user",
        user_type="agent",
    ...
    campaign.agent_assignments.append(
        CampaignAgent(agent_id=agent.id, role=CampaignRole.PARTICIPANT.value)
    )
    db.add(campaign)
    await db.commit()
    standard = await create_standard(
        db, campaign.id, agent.id, "Pinned Standard", "E2E standard", _two_block_standard()
    )
...
async def _seed_canonical_artifacts(
    db: AsyncSession, session: Session, version
    ...
    await db.flush()
    transcript = [
        {"speaker": row.speaker, "text": row.utterance_text, "sequence_number": row.sequence_number}
    ...
    canonical = canonical.model_copy(
        update={
            "recommendations": build_rubric_recommendations(
                canonical, version.snapshot, version.id, version.version_number
            )
        }
    )
    evaluation_result = EvaluationResult(
        ...
        strengths=[
            StrengthItem(
                description="Clear greeting",
                category=EvaluationCategory.CALL_OPENING,
                transcript_excerpt="Hello, this is Alex",
            )
        ],
        weaknesses=[
            WeaknessItem(
                description="Rushed resolution",
    ...
    serialized_coaching["_rubric_recommendations"] = [
        item.model_dump(mode="json") for item in coaching.rubric_recommendations
    ]
    ...
    db.add(
        CoachingReport(
            ...
            total_mistakes=coaching.total_mistakes,
            no_mistakes=coaching.no_mistakes,
        )
    )
    await db.commit()
    await LearningPlanGenerator().generate_and_persist(
        evaluation_result, session.id, session.agent_id, db=db
    )
    return canonical
...
async def _reload_session(db: AsyncSession, session_id: uuid.UUID) -> Session:
    result = await db.execute(
        select(Session)
        .options(selectinload(Session.campaign), selectinload(Session.negotiation_standard_version))
...
async def _seed_report_variant(db: AsyncSession, *, variant: str) -> tuple[User, Session]:
    ...
    if variant == "legacy":
        db.add(
            Evaluation(
                session_id=session.id,
                overall_score=60,
                category_scores=[{"category": "call_opening", "score": 60}],
                strengths=[],
                weaknesses=[],
                is_too_short=False,
            )
    ...
    db.add(
        CoachingReport(
            session_id=session.id, mistakes_by_category={}, total_mistakes=0, no_mistakes=True
        )
    )
    db.add(
        LearningPlan(
            session_id=session.id, agent_id=user.id, weak_competencies=[], all_passing=True
        )
    )
    await db.commit()
    return user, session
...
async def test_authenticated_session_report_full_lifecycle(e2e_client, async_db, monkeypatch):
    ...
    transcript_response = await e2e_client.get(
        f"/api/sessions/{session.id}/transcript", headers=headers
    )
    ...
    assert [
        item["penalty_total"] for item in payload_v1["evaluation"]["canonical"]["categories"]
    ] == [10, 15]
    assert payload_v1["evaluation"]["canonical"]["categories"][1]["failed_criteria"] == ["no-plan"]
    assert (
        payload_v1["evaluation"]["canonical"]["categories"][0]["evidence"][0]["sequence_number"]
        == 0
    )
    assert [block["rubric_block_id"] for block in payload_v1["coaching"]["blocks"]] == [
        "opening",
        "resolution",
    ]
    assert payload_v1["coaching"]["blocks"][0]["recommendations"][0]["source_speaker"] == "agent"
    assert all(
        item["scenario_id"] == str(scenario.id) for item in payload_v1["learning_plan"]["items"]
    )
    ...
    for export_format, (media_type, marker) in export_expectations.items():
        exported = await e2e_client.get(
            f"/api/sessions/{session.id}/report/export?format={export_format}", headers=headers
        )
    ...
    row_v1 = (
        await async_db.execute(
            select(SessionReport).where(
                SessionReport.session_id == session.id, SessionReport.report_version == 1
            )
        )
    ).scalar_one()
    immutable_v1 = {
        "payload": copy.deepcopy(row_v1.payload),
        "content_hash": row_v1.content_hash,
        "status": row_v1.status,
        "generated_by": row_v1.generated_by,
        "created_at": row_v1.created_at,
        "updated_at": row_v1.updated_at,
    }
    ...
    assert {
        "payload": copy.deepcopy(row_v1_after.payload),
        "content_hash": row_v1_after.content_hash,
        "status": row_v1_after.status,
        "generated_by": row_v1_after.generated_by,
        "created_at": row_v1_after.created_at,
        "updated_at": row_v1_after.updated_at,
    } == immutable_v1
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(side_effect=RuntimeError("must not leak")),
    )
    failed_regeneration = await e2e_client.post(
        f"/api/sessions/{session.id}/report", headers=headers
    )
    ...
    failed_status = await e2e_client.get(
        f"/api/sessions/{session.id}/report/status", headers=headers
    )
...
async def test_report_status_terminal_fixtures(e2e_client, async_db, variant, expected_status):
    ...
    report = await generate_report(
        async_db, await _reload_session(async_db, session.id), generated_by=user.id
    )
