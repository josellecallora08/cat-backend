        ...
        "violations": [
            {
                ...
                "explanation": "Rude finding.",
                "evidence_sequence_numbers": [item],
            }
            for item in sequences[:violation_count]
        ],
        "failed_criteria": [],
...
def _validated(
    ...
    order: tuple[str, str] = ("opening", "resolution"),
):
    categories = {
    ...
    transcript = [
        {"sequence_number": item, "speaker": "agent", "text": "Line"} for item in range(1, 6)
    ]
...
def test_penalized_scores_and_weighted_totals_stay_in_bounds(
    raw_opening: int, raw_resolution: int, violation_count: int
) -> None:
    result = calculate_rubric_score(_validated(raw_opening, raw_resolution, violation_count))
    assert 0 <= result.weighted_total <= 100
    assert all(
        0 <= category.penalized_score <= 100
        for category in result.categories
        if category.penalized_score is not None
    )
...
def test_category_input_permutation_does_not_change_serialized_score(
    opening_score: int, resolution_score: int
) -> None:
    first = calculate_rubric_score(
        _validated(opening_score, resolution_score, order=("opening", "resolution"))
    )
    second = calculate_rubric_score(
        _validated(opening_score, resolution_score, order=("resolution", "opening"))
    )
...
def test_penalties_never_increase_a_score(raw_opening: int, violation_count: int) -> None:
    ...
    assert (
        with_penalty.categories[0].penalized_score <= without_penalty.categories[0].penalized_score
    )


@given(st.integers(min_value=0, max_value=100), st.integers(min_value=0, max_value=5))
def test_same_snapshot_and_observation_are_serialization_deterministic(
    raw_opening: int, violation_count: int
) -> None:
